...
The **GitHub Action** runs `netlify env:set` for **`NEXTAUTH_SECRET`**, **`AUTH_SECRET`** (same value as `NEXTAUTH_SECRET`), **`NEXTAUTH_URL`**, **`DATABASE_URL`**, **`DIRECT_URL`**, and **`AUTH_TRUST_HOST=true`** on **`deploy-preview`**, **`branch-deploy`**, and **`branch:<PR head ref>`** before `netlify deploy --build`. CLI deploys with **`--alias pr-N`** are not Git “deploy previews”; Netlify often resolves a different context at runtime, so Functions may not see **`deploy-preview`**-only variables (empty `CONTEXT`, missing `DATABASE_URL`). Mirroring the same values to **`branch-deploy`** and the PR branch context fixes that. Use **`env:unset` only with those preview contexts** (never site-wide) so production is not deleted.
