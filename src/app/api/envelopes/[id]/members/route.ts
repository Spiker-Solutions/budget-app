...
export async function POST(
  ...
  try {
    const isAdmin = await checkEnvelopeAdmin(envelopeId, session.user.id);

    if (!isAdmin) {
      return NextResponse.json(
        errorResponse("Only admins and owners can invite members"),
        { status: 403 }
      );
    }
    ...
    const existingMembership = await prisma.envelopeUser.findUnique({
      where: {
        userId_envelopeId: {
          userId: user.id,
          envelopeId,
    ...
    if (existingMembership) {
      return NextResponse.json(
        errorResponse("User is already a member of this envelope"),
        { status: 400 }
      );
    }

    const membership = await prisma.envelopeUser.create({
      data: {
        userId: user.id,
        envelopeId,
        role,
...
export async function DELETE(
  ...
  try {
    ...
    const targetMember = await prisma.envelopeUser.findUnique({
      where: {
        userId_envelopeId: {
          userId,
          envelopeId,
    ...
    await prisma.envelopeUser.delete({
      where: {
        userId_envelopeId: {
          userId,
          envelopeId,
